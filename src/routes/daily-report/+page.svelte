<script lang="ts">
	...
	let summaries: TodaySummary[] = data.summaries;
	let dailySummary: DailySummary | null = data.dailySummary;
	let selectedDate: string = data.date;
...
<svelte:head>
	<title>Daily Report - {formatDate(selectedDate)} - youtubegist</title>
	<meta name="description" content="Daily report for videos watched on {formatDate(selectedDate)} on youtubegist." />
</svelte:head>
...
<main class="container mx-auto max-w-3xl px-4 py-8">
	<div class="flex items-start justify-between gap-4">
		<div>
			<div class="flex items-center gap-2 mb-2">
				...
				<CalendarIcon class="h-5 w-5 text-zinc-400" />
				<h1 class="text-3xl font-semibold text-zinc-100">Daily Report</h1>
			</div>
			<p class="text-sm text-zinc-400">Videos summarized on {formatDate(selectedDate)}</p>
		</div>
	</div>
	...
	{#if summaries.length === 0}
		<div class="mt-16 rounded-xl border border-dashed border-zinc-700/60 bg-zinc-900/40 p-8 text-center">
			<p class="text-lg text-zinc-400">No videos summarized on {formatDate(selectedDate)}. Try summarizing a YouTube video!</p>
		</div>
	{:else}
